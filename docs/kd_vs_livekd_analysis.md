...
> **⚠️ 重要：系统上有两个完全不同来源的 kd.exe，用错的那个会因权限问题无法启动！**

#### 来源一：Windows SDK（✅ 推荐！实战专用！）

```
安装方式：Windows SDK AddOn (版本 10.1.0.0)
安装路径：C:\Program Files (x86)\Windows Kits\10\Debuggers\x64\
调用命令：sudo "C:/Program Files (x86)/Windows Kits/10/Debuggers/x64/kd.exe" -kl -c "..."
kd 版本：10.0.26100.2454
```

**这是唯一能在 Claude Code / 自动化脚本中可靠调用的版本。**
无 ACL 限制、无 Store sandbox、sudo 提权即可用。

#### 来源二：Microsoft Store WinDbg Preview（⛔ 不要在自动化中使用！）

```
安装方式：Microsoft Store 自动安装/更新
安装路径：C:\Program Files\WindowsApps\Microsoft.WinDbg_1.2601.12001.0_x64__8wekyb3d8bbwe\amd64\
WinDbg 版本：1.2601.12001.0（可更新到 1.2603.20001.0）
```

> **⛔ WindowsApps 目录有 ACL 锁死！**
> - 普通用户无法进入、列出、执行该目录下的任何文件
> - `where.exe kd.exe` 找不到它（不在 PATH 中）
> - Claude Code 的 Bash 工具因 Codex 沙箱权限问题**完全无法调用**
> - 即使用 sudo 也可能因为 Store 应用的 AppContainer 限制而失败
>
> **只能用于：** 手动双击启动 WinDbg Preview GUI 做交互式调试

#### 两者对比

| | Windows SDK 版 | Microsoft Store 版 |
|--|---------------|---------------------|
| 路径 | `...\Windows Kits\10\Debuggers\x64\kd.exe` | `...\WindowsApps\Microsoft.WinDbg_...\amd64\kd.exe` |
| 版本 | 10.0.26100.2454 | 1.2601.12001.0 |
| 调试引擎 | dbgeng.dll (相同) | dbgeng.dll (相同) |
| 符号解析 | dbghelp.dll (相同) | dbghelp.dll (相同) |
| 自动化调用 | **✅ 可以** | **⛔ 不行（ACL 锁死）** |
| sudo 提权 | **✅ 正常** | **⛔ AppContainer 限制** |
| Claude Code | **✅ 完美** | **⛔ 完全不可用** |
| 适用场景 | 脚本/CLI/ObMaster 集成 | 手动 GUI 调试 |
...
# ⛔ 错误用法 — 用户目录的旧副本（可能版本不对，之前因此翻车过）
sudo C:/Users/asus/kd.exe -kl -c "..."
# 报错：Local kernel debugging is disabled（因为是旧版 SDK 不支持 debug on）
```
...
- **永远用 SDK 版 kd.exe**，不要用 Store 版或用户目录的旧副本
- `-kl` 是 local kernel debug，只读不写，不会修改内核状态

## 11. sp-test BSOD 根因：大页误判为 PTE 只读（乌龙事件，2026-04-11）

### 现象

`/sp-test <addr>` 在 Stage 2（no-op PTE write）触发 BSOD 0x50：
```
BugCheck 0x00000050 (PAGE_FAULT_IN_NONPAGED_AREA)
P1: 0xFFFFC1FC02B268C4  (Faulting VA — 自映射区域内)
P2: 0x0000000000000003  (WRITE 操作)
P3: 0xFFFFF805B8F8154F  (Faulting RIP — RTCore64 Wr32 路径)
P4: 0x0000000000000008  (Page table level fault)
```

### ~~错误的分析（已推翻）~~

~~P2=3（WRITE）+ P4=8（page table level）= PTE 页面本身是只读的。~~
~~Win10 19041+ 内核把自映射区域的 PTE 页面标记为 Read-Only。~~

### 真正的根因：大页没有 PTE 层

测试地址（ntoskrnl / MiGetPteAddress）在 **2MB 大页**上（PDE.PS=1）。
大页只有 PML4 → PDPT → PD 三层，**没有 PT 层**。

`PteVaOf(addr)` 对大页地址算出的结果不是 PTE 地址——它落在自映射的
PDE 区域中某个**不属于任何有效 PTE 的位置**。`Wr32` 写这个地址 → page fault。

```
正常 4KB 页:  PteVaOf(addr) → 有效 PTE 地址 → 可读可写 ✓
大页 2MB:     PteVaOf(addr) → PDE 区域垃圾地址 → 未映射或只读 → BSOD ✗
```

**不是"PTE 页只读"，是"根本没有 PTE，写错了地址"。**

### 修复后验证

修复：Stage 1 加 `IsLargePage(addr)` 检查，大页直接 SKIP。

用 ksafecenter64.sys（4KB 页）测试，kd 确认 PTE 存在：
```
PDE  = 0x0A00000109115863  → PS=0 (4KB) ✓
PTE  = 0x8900000517169121  → Present=1, PA=0x517169000 ✓
```

sp-test 三个 Stage 全部 PASS：
```
Stage 0: HVCI 未启用 ✅
Stage 1: PTE 读取成功 (4KB, Present) ✅
Stage 2: PTE 写回成功 (readback 匹配) ✅
Stage 3: Shadow page swap → shadow PA 验证 → restore → 原始 PA 验证 ✅
```

**结论：4KB 页的 PTE 是可写的。WritePte 机制完全正常。之前的"PTE 页只读"分析是乌龙——
问题从头到尾都是大页检查缺失。**

### 教训

1. 蓝屏分析不能只看 Faulting VA 在自映射区域就断定"PTE 页只读"
2. P4=8（page table level）的真正含义是页表遍历时某一层缺失，不等于"有页但只读"
3. 大页是 safepatch 的天敌——没有 PTE 层就无法做 shadow page swap
4. ntoskrnl 在大页上，第三方驱动（ksafecenter64 等）在 4KB 页上——safepatch 目标应选后者
