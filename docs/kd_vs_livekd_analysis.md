...
## 11. sp-test BSOD 根因：PTE 页只读 (2026-04-11)
...
Faulting VA `0xFFFFC1FC02B268C4` 在 MmPteBase (`0xFFFFC180`) 的自映射区域内，
PML4 index = 387 = MmPteBase 的自映射 index。这是 `WritePte` 的 `Wr32(pteVA, ...)`
尝试写入一个 PTE entry 的地址。

**P2=3（WRITE）+ P4=8（page table level）= PTE 页面本身是只读的。**

Win10 19041+ 内核把自映射区域的 PTE 页面标记为 Read-Only。RTCore64 的 `Wr32` 是
裸虚拟地址写入，走 CPU 页表权限检查，写只读页 → #PF → BSOD。

### 页表权限链验证

```python
PML4E = 0x0000000010009063  → Present=1, Write=1 ✓
PDPTE = 0x0A000005C98C5863  → Present=1, Write=1 ✓
PDE   = 0x0A00000109115863  → Present=1, Write=1 ✓, PS=0 (4KB)
PTE   = ???                  → 这就是要写的目标，但页面本身不可写
```

PDE 的 Write bit 是 1，说明保护不是来自 PDE 层。更可能是 PTE 页面的
PTE entry 本身（自映射的 PTE of PTE）有 Write=0，或 HVCI/VBS 在虚拟化层拦截。

### 解决方向

1. **物理地址写入** — 绕过虚拟地址页表权限，直接写 PTE 的物理地址
2. **PDE Write bit 升级** — 递归往上找到只读层，临时设为可写
3. **MDL 映射** — 通过 IoAllocateMdl + MmMapLockedPages 重映射 PTE 页为可写
4. **寻找有物理写入能力的驱动** — RTCore64 的 MmMapIoSpace 不能映射 RAM 页

### 当前状态

WritePte 在本机环境可能可用（需要验证 PTE 页权限），在网咖实战环境确认 BSOD。
`/sp-test` 的 Stage 2 和 Stage 3 缺少大页检查也是蓝屏因素之一。
