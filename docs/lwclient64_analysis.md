...
**通信库：** `kscdrv64.dll`（B:\lwclient64\，160 imports/8 libs，备份在 D:\lwclient64_backup\）

ppm 分析 kscdrv64.dll 提取的完整 IOCTL 调用序列（sub_1B930 主控制循环）：

| 顺序 | IOCTL Code | 用途（推测） |
|------|-----------|-------------|
| 1 | `0x220019` | 初始化/注册（传模块路径 + 进程信息） |
| 2 | `0x22000C` 或 `0x220014` | 控制命令（条件分支） |
| 3 | `0x220024` 或 `0x220028` | 控制命令（条件分支） |
| 4 | `0x220008` | 传进程信息 |
| 5 | `0x220010` | 传进程信息 |
| 6 | `0x220004` | 传进程信息 |
| 7 | `0x22002C` | 后续控制 |
...
其他 IOCTL（sub_1490 异步事件监听）：
- `0x17003D` — 普通模式事件等待
- `0x12083D` — 备用模式事件等待
- `0x120894` — sub_1C90 使用

**注意：** `ForceRemove` / `NoRemove` / `Delete` 字符串是 ATL COM 注册宏，不是卸载命令。
...
## lwclient64.exe 实验机实测（2026-04-12）

### 启动测试

从 `D:\lwclient64_backup\` 直接运行 lwclient64.exe，日志关键行：

```
Load safecenter success                    ← kscdrv64.dll 成功打开 \\.\SafeCenter 并发了 IOCTL
ScsiDisk Query faild(ret:2)                ← \\.\KScsiDisk 设备不存在（驱动被 nuke 过）
safe option ip:192.168.10.241,port:13501   ← 连云更新服务器（实验机无服务器，心跳失败）
Heart err                                  ← 心跳超时
```

**确认：** IOCTL 通道在实验机上可用。kscdrv64.dll 的 `SafeCenterStart` 成功连接 ksafecenter64 驱动。

### 观察

- lwclient64.exe 启动后 ksafecenter64 的回调**没有重新注册**（nuke 已清除）
- lwclient64.exe 退出后驱动状态无变化（仍 UNLOAD_INVOKED，文件仍锁）
- lwclient64.exe 被 ksafecenter64 的 ObCallback 保护——taskkill 返回 ACCESS_DENIED
- **结论：** lwclient64.exe 只做启动流程（SafeCenterStart），不做卸载流程
- 卸载入口可能在厂商的安装包或独立卸载工具中，不在 lwclient64.exe 里

### 进程保护验证

```
taskkill /F /PID 15316    → ACCESS_DENIED（ObCallback 拦截）
ObMaster /kill 15316      → err=5（ObCallback 拦截 OpenProcess）
ObMaster /kill-ppl 15316  → 进程已退出（用户手动 Ctrl+C 关闭）
```

即使驱动被 nuke（回调清零、MajorFunction 全部 ret stub），**nuke 前注册的 ObCallback 仍在内核的 ObjectType CallbackList 链表中生效**——因为 nuke 步骤 1-2 只清理 Notify 和 CmCallback 数组/链表，没有清理 ObCallback 链表。

**教训：** 完整的 nuke 应该也包括 ObCallback 清理。当前版本遗漏了这一步。

### 厂商卸载程序逆向（NSIS Un_A.exe）

卸载程序：`B:\lwclient64\uninst64.exe`（运行时自解压到 `~nsuA.tmp\Un_A.exe`）。
NSIS 生成，PE32 x86（名字叫 64 但实际是 32 位）。

**7z 解包内容：**
```
$PLUGINSDIR/
├── plugin.dll      ← 自定义 NSIS 插件（关键）
├── nsExec.dll      ← NSIS 命令执行插件（调 sc stop 等）
├── System.dll      ← NSIS 系统调用插件
└── modern-header.bmp
```

**plugin.dll 分析（PDB: E:\kygx2019\trunk\source\tool\install\nsis_script\PluginDll\plugin.pdb）：**

| 导出函数 | 功能 |
|----------|------|
| `FindProc` | 检查指定进程是否存在（CreateToolhelp32Snapshot 遍历） |
| `TerminProc` | 杀死指定进程（OpenProcess + TerminateProcess） |
| `GetIps` | 获取网卡 IP 地址（GetAdaptersInfo） |
| `GenCRC` | CRC 校验 |

导入：OpenProcess、TerminateProcess、Process32FirstW/NextW、AdjustTokenPrivileges（提权杀进程）。
**无 DeviceIoControl** — plugin.dll 不通过 IOCTL 通知驱动。

**nsExec.dll** 是 NSIS 标准插件，导出 `ExecToLog` 和 `ExecToStack`，可执行任意命令行。

**推测的 NSIS 卸载脚本流程：**
```nsis
; 1. 杀云更新进程
plugin::TerminProc "lwclient64.exe"
plugin::TerminProc "kssd.exe"
plugin::TerminProc "lwhardware64.exe"
; ...

; 2. 通过 nsExec 停止和删除驱动服务
nsExec::ExecToLog "sc stop ksafecenter"
nsExec::ExecToLog "sc delete ksafecenter"
nsExec::ExecToLog "sc stop KScsiDisk"
nsExec::ExecToLog "sc delete KScsiDisk"
; ...

; 3. 删文件（NSIS 内置命令）
Delete "B:\lwclient64\*.exe"
RMDir /r "B:\lwclient64\skin"
; 锁定的文件用 Delete /REBOOTOK
Delete /REBOOTOK "$SYSDIR\drivers\ksafecenter64.sys"
; ...

; 4. 删注册表
DeleteRegKey HKLM "SYSTEM\CurrentControlSet\Services\ksafecenter"
; ...

; 5. 提示重启
MessageBox "需要重启计算机以完成卸载"
```

**NSIS 脚本反编译成功（nrs + 7z 解包 → string table 提取）：**

实际卸载脚本关键操作：

```
; 1. 服务控制（通过 System.dll 调 advapi32 API）
advapi32::ControlService(handle, 1, ...)     ; SERVICE_CONTROL_STOP
advapi32::ControlService(handle, 2, ...)     ; SERVICE_CONTROL_PAUSE
advapi32::ControlService(handle, 3, ...)     ; SERVICE_CONTROL_CONTINUE
advapi32::QueryServiceStatus / QueryServiceStatusEx  ; 等待 stopped
advapi32::DeleteService(handle)              ; 删除服务

; 2. ⚠ 关键：用 devcon 移除 PnP 设备
Exec "devcon64.exe remove kscsidiskadapter"  ; PnP 设备移除 → 触发 IoDeleteDevice

; 3. 删除注册表
DeleteRegKey SYSTEM\CurrentControlSet\Services\KScsiDisk
DeleteRegKey SYSTEM\CurrentControlSet\Services\KScsiDisk\Enum
DeleteRegKey SYSTEM\CurrentControlSet\Services\KScsiDisk\Param

; 4. VNC 工具清理
Exec "kvnserver.exe -remove -silent"

; 5. 删除所有驱动 .sys 和 DLL 文件
Delete ksafecenter64.sys, kscsidisk64.sys, kshutdown64.sys, kdisk64.sys
Delete krestore64.sys, kcachec64.sys, kpowershutdown64.sys, kantiarp64.sys
Delete kshut64.dll, kshut.dll

; 6. plugin.dll 杀进程
plugin::TerminProc "lwclient64.exe" (等)
```

**关键发现：`devcon64.exe remove kscsidiskadapter`！**

卸载程序用 **devcon**（Windows 设备管理工具）的 `remove` 命令移除 KScsiDisk 的 PnP 适配器设备。
这才是触发 `IoDeleteDevice` 的正确路径——不是 IOCTL，不是手动内核操作，是 **PnP remove**！

devcon remove 会：
1. 发送 `IRP_MN_REMOVE_DEVICE` 到设备栈
2. 驱动的 PnP handler 调 `IoDeleteDevice`
3. DeviceObject 引用清零
4. 如果 UNLOAD_INVOKED 已设 → IopDeleteDriver → MmUnloadSystemImage
5. 文件锁释放 → 可重新加载

**对于 ObMaster 来说，正确的流程：**
```
1. /nuke-driver 清理回调（Notify + CmCallback）
2. devcon remove kscsidiskadapter（PnP 移除设备 → IoDeleteDevice）
3. sc stop + sc delete（停止并删除服务）
4. 驱动干净卸载，无需重启
```

**厂商完整卸载策略（脚本实证）：**
```
1. plugin::TerminProc 杀所有云更新用户态进程
2. advapi32::ControlService(STOP) 尝试停止（可能失败但不影响后续）
3. devcon64.exe remove kscsidiskadapter（PnP 移除 KScsiDisk 设备）
4. advapi32::DeleteService 删除服务注册表条目
5. NSIS Delete 删除 .sys / .dll 文件（锁定的用 /REBOOTOK）
6. kvnserver.exe -remove -silent（清理 VNC）
7. 提示重启（仅用于清理 /REBOOTOK 标记的残留文件）
```
...
运行官方卸载程序后：
- 提示需要重启（用户拒绝）
- `D:\lwclient64_backup\` 从 70+ 文件减少到 12 个核心文件
- 保留的文件：lwclient64.exe、kscdrv64.dll、kclient64.dll、kcommon64.dll、kicloud64.dll + 依赖 DLL + config/log/lanp2p64 目录
- 驱动状态：仍然 Stopped，文件仍锁着
- `PendingFileRenameOperations` 中没有 ksafe 相关条目

**结论：厂商的卸载程序也需要重启才能完全释放驱动文件。**
这不是 ObMaster 的 nuke-driver 的缺陷——是 Windows 内核的限制：
一旦驱动代码页被 MmLoadSystemImage 映射，只有 MmUnloadSystemImage 能释放，
而 MmUnloadSystemImage 需要所有 DeviceObject 和引用完全清零后才会被 IopDeleteDriver 调用。

**对于 VBox 实战场景，nuke-driver 足够用：**
1. 清理所有回调（阻止 evil handle 和 APC 注入）
2. 驱动功能性死亡（MajorFunction 全 ret stub）
3. VBox 启动成功
4. 重启后驱动被云更新还原系统自动重装（还原卡特性）

不需要运行时干净卸载——VBox 只需要驱动"不干活"就行。
